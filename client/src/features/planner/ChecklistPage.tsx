...
export function ChecklistPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="grid gap-3 md:grid-cols-[1fr_220px_auto]"><Input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Add item" /><Input value={category} onChange={(event) => setCategory(event.target.value)} placeholder="Category" /><Button onClick={addItem}><Plus className="h-4 w-4" /> Add</Button></Card>
